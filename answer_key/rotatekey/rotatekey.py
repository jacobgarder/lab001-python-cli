...
@click.group(help="Utilities for rotating network secrets and keys.")
...
def cli(ctx, inventory, debug, cli_verbose, prefer_restconf):
    ...
    if "NETWORK_USERNAME" not in os.environ or "NETWORK_PASSWORD" not in os.environ:
        ...
        click.secho(
            "ERROR: You must set the NETWORK_USERNAME and NETWORK_PASSWORD environment variables.",
            fg="red",
        )
        exit(1)
...
@click.command(name="check-inventory", help="Display status of communication protocols for each device in inventory.")
@click.pass_context
...
@click.group(help="Manipulate the SNMP communities configured on network devices.")
def snmp():
    """
...
#       - Set a good help message for command an all options
#       - Ensure the command has access to the command context object
@click.command(name="list", help="Lookup and list the SNMP communities created on the devices in inventory.")
@click.pass_context
def snmp_list(ctx):
    """
    ...
    Args:
        ctx (Click.Context):
    """
    click.secho(f"{'Device':15} {'Community':15} {'Rights':5}")
    click.secho("-" * 40)
    for device in ctx.obj["inventory"]:
        ...
        for snmp in current_snmp:
            click.secho(f"{device['device_name']:15} {snmp['name']:15} {snmp['permission']:5}")
...
@click.command(name="update", help="Update the SNMP community strings configured on devices in the inventory.")
@click.pass_context
@click.option(
    "-d",
    "--delete-current",
    is_flag=True,
    help="Whether to delete all current SNMP communities",
)
...
def snmp_update(ctx, delete_current: bool, ro_community: str, rw_community: str):
    ...
    Returns:
    """
    click.secho("Updating the network devices to: ")
    if delete_current:
        # TODO: The following message should be printed to the screen in red text
        click.secho(
            "  - All currently configured SNMP community strings will be removed",
            fg="red",
        )
    if ro_community:
        # TODO: The following message should be printed to the screen in blue text
        click.secho(
            f"  - A new read-only community string '{ro_community}' will be created",
            fg="blue",
        )
    if rw_community:
        # TODO: The following message should be printed to the screen in green text
        click.secho(
            f"  - A new read-write community string '{rw_community}' will be created",
            fg="green",
        )
...
# Add top level commands to the main cli command/entrypoint
cli.add_command(check_inventory)
cli.add_command(snmp)

# Add subcommands to the snmp command
snmp.add_command(snmp_list)
snmp.add_command(snmp_update)
